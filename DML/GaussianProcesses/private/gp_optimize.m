function [X fX i] = gp_optimize(X, f, length, varargin)
%GP_OPTIMIZE Minimize differentiable multivariate function using conjugate gradients.
%   [X fX i] = GP_OPTIMIZE(X, f, l, P1, P2, ...)
%    X    initial guess; may be of any type, including struct and cell array
%    f    pointer to the function to be minimized. The function
...
%    l    length of the run; if it is positive, it gives the maximum number of
...
%   Author: C. E. Rasmussen (http://gaussianprocess.org/gpml/code),
...
% either with an error or returning Nan or Inf, and this function should
% handle that gracefully.

printf('\tGP optimization');
...
i = 0; % Initialize run length counter
ls_failed = 0; % No previous line search has failed
[f0 df0] = feval(f, X, varargin{:}); % Get function value and gradient
Z = X; X = unwrap(X); df0 = unwrap(df0);
%printf('%s #i (current value: %4.1e)\n', S, i, f0); % Long display
printf('.'); % Short display

fX = f0;
i = i + (length < 0); % Count 1 function evaluation
...
v = [];   
if isnumeric(s) % Numeric values
  v = s(:); % Recast to column vector
elseif isstruct(s) % Structure
  v = unwrap(struct2cell(orderfields(s))); % Alphabetize, conv to cell, recurse
elseif iscell(s) % Cell array
  for i = 1:numel(s) % For each element of the cell array
    v = [v; unwrap(s{i})];
  end
